...
describe("命令 hook 适配器（DESIGN 13）", () => {
  ...
  it("PreToolUse：命令回 verdict 裁决", async () => {
    ...
    const verdict = await handler({ type: "PreToolUse", toolCallId: "t1", toolName: "bash", input: {}, agentPath: "/root" });
  ...
  it("PreToolUse：allow / ask 透传", async () => {
    ...
    expect(await allow({ type: "PreToolUse", toolCallId: "t1", toolName: "read", input: {}, agentPath: "/root" })).toBe("allow");
  ...
  it("PreToolUse：非零退出 / 非 JSON 输出 / 超时 → 保守 deny", async () => {
    ...
    expect(await failing({ type: "PreToolUse", toolCallId: "t1", toolName: "bash", input: {}, agentPath: "/root" })).toBe("deny");
    // 输出非 JSON
    const garbage = createCommandHook(makeHookCommand(dir, `process.stdout.write("hello");`), { timeoutMs: 5000 });
    expect(await garbage({ type: "PreToolUse", toolCallId: "t1", toolName: "bash", input: {}, agentPath: "/root" })).toBe("deny");
    ...
    expect(await slow({ type: "PreToolUse", toolCallId: "t1", toolName: "bash", input: {}, agentPath: "/root" })).toBe("deny");
  });
  ...
  it("观测事件（PostToolUse 等）：命令被调用，返回 undefined", async () => {
    ...
    const handler = createCommandHook(command, { timeoutMs: 5000 });
    const result = await handler({ type: "PostToolUse", toolCallId: "t1", toolName: "read", input: {}, output: "x", isError: false, agentPath: "/root" });
...
describe("HookBus 与命令装配", () => {
  it("buildHookBus 语义：config.hooks 逐事件注册命令", async () => {
    ...
    const results = await bus.emit({ type: "PreToolUse", toolCallId: "t1", toolName: "bash", input: {}, agentPath: "/root" });
